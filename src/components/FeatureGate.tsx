...
interface FeatureGateProps {
  feature: FeatureKey
  requiredTier?: SubscriptionTier   // shown in upgrade prompt; auto-derived if omitted
  children: ReactNode
  fallback?: ReactNode              // custom fallback; defaults to upgrade card
}
...
export function FeatureGate({ feature, requiredTier = 'pro', children, fallback }: FeatureGateProps) {
  ...
  if (isLoading) {
    return (
      <div className="animate-pulse bg-white/5 rounded-xl h-24 w-full" />
    )
  }
      ...
      <div className="px-6 py-5 flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Upgrade to {plan.name} to unlock this and more.
          </p>
        </div>

        <Link
          href="/dashboard/billing"
          className="shrink-0 bg-gold-500 hover:bg-gold-400 text-navy-950 font-bold
                     text-xs px-4 py-2 rounded-lg transition-colors whitespace-nowrap"
        >
          Upgrade →
        </Link>
      </div>
...
// ── Usage-limit wrapper ────────────────────────────────────────────────────────
// For free-tier metered features (e.g. 5 VA checks / mo)
interface UsageLimitGateProps {
  used: number
  limit: number
  feature: FeatureKey      // unlimited version of this feature
  children: ReactNode
  noun?: string            // e.g. "VA check" → "VA checks"
}

export function UsageLimitGate({
  used, limit, feature, children, noun = 'use',
}: UsageLimitGateProps) {
  const { can, isLoading } = useSubscription()

  if (isLoading) return <div className="animate-pulse bg-white/5 rounded-xl h-24 w-full" />

  // Pro/Elite bypasses the meter
  if (can(feature)) return <>{children}</>
    ...
    return (
      <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 px-6 py-5">
        ...
        <p className="text-xs text-slate-400 mb-4">
          You've used all {limit} free {noun}s this month.
          Upgrade for unlimited access.
        </p>
        <Link
          href="/dashboard/billing"
          className="inline-block bg-gold-500 hover:bg-gold-400 text-navy-950 font-bold
    ...
    <div>
      {remaining <= 2 && (
        <div className="mb-3 flex items-center justify-between px-4 py-2 bg-amber-500/10 border border-amber-500/20 rounded-lg">
          ...
          <Link href="/dashboard/billing" className="text-xs text-gold-500 hover:text-gold-400 font-medium">
            Upgrade
          </Link>
        </div>
      )}
